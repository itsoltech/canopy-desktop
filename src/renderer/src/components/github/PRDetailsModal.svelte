<script lang="ts">
  ...
  // `.otherwise` rather than `.exhaustive`: `reviewDecision` is `string | null`,
  // not a literal union.
  ...
  function reviewerChip(state: string): { label: string; cls: string } {
    // `.otherwise` rather than `.exhaustive`: review states arrive as free-form
    // strings from the GitHub API, not a literal union.
    return match(state)
      .with('APPROVED', () => ({ label: 'approved', cls: 'bg-success-bg text-success-text' }))
      .with('CHANGES_REQUESTED', () => ({
        label: 'changes requested',
