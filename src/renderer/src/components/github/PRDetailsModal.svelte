<script lang="ts">
  ...
  let reviewChip = $derived.by(() =>
    // `.otherwise` (not `.exhaustive`): reviewDecision comes from the GitHub API,
    // so an unrecognised value must degrade to "no chip" rather than throw.
  ...
  function reviewerChip(state: string): { label: string; cls: string } {
    // `.otherwise` (not `.exhaustive`): `state` is an unconstrained string from the
    // GitHub API, and anything unrecognised reads as a plain comment.
    return match(state)
      .with('APPROVED', () => ({ label: 'approved', cls: 'bg-success-bg text-success-text' }))
      .with('CHANGES_REQUESTED', () => ({
        label: 'changes requested',
