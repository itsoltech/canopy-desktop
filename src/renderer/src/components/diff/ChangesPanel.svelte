...
<div class="flex flex-col h-full">
  ...
  {:else if loading}
    <!-- Without this branch the panel renders nothing while the diff loads, so
         an in-flight fetch is indistinguishable from a clean worktree. -->
    <div class="flex items-center justify-center h-full p-4">
      <span class="text-sm text-text-faint" role="status">Loading changes…</span>
    </div>
  {:else}
    <div class="flex items-center justify-center h-full p-4">
