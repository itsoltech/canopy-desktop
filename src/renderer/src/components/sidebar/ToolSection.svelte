<script lang="ts">
  ...
  // Apply the user's sidebar view config: keep only visible + available tools,
  // in the configured order. AI and non-AI tools share one ordered list so the
  // user can sort them freely.
  let orderedTools = $derived(
    getToolView()
      .filter((e) => e.visible)
      .map((e) => toolsById.get(e.id))
      .filter((t): t is NonNullable<typeof t> => t !== undefined)
      .filter((t) => getToolAvailability()[t.id] !== false),
  )
