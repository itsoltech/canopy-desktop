...
- **Repository Configuration** (`.canopy/config.json`) allows per-project settings like task tracker board IDs and branch/PR templates. This file is intended to be committed to version control. Saves publish a unique same-directory temporary file with `rename`, so concurrent readers observe either the previous or next complete JSON; transient Windows destination locks receive a bounded retry. Old crash leftovers are cleaned on a later save after 24 hours. This prevents partial reads but does not claim `fsync` durability across power loss.
