...
- **Repository Configuration** (`.canopy/config.json`) allows per-project settings like task tracker board IDs and branch/PR templates. This file is intended to be committed to version control. Reads and final pretty-printed writes are capped at 1 MiB; an oversized save fails before touching the existing file. Saves publish a unique same-directory temporary file with `rename`, so concurrent readers observe either the previous or next complete JSON; transient Windows destination locks receive a bounded retry. First-time initialization publishes its complete temporary file through an exclusive same-directory hard link, which cannot replace a config created concurrently. Old crash leftovers are cleaned on a later write after 24 hours. Only `ENOENT` means "missing": oversized, invalid or unreadable files block global fallback and initialization. This prevents partial reads and accidental replacement but does not claim `fsync` durability across power loss.
- **Global Configuration** (`taskTracker.globalConfig` in the SQLite preferences store) provides a fallback for repository-level settings across all projects on a machine. It is a preference row rather than a file, so the publication contract above does not apply.
