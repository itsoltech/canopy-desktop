...
- **Reset to default** — removes the project value from `.canopy/config.json` so the **built-in** template applies (there is no other tier). **Remove project override** asks for confirmation, then drops a project-specific override so tasks from that project fall back to the base template.
