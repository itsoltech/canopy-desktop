...
Activity polling reads one newest-first page of up to 100 repository runs. A truncated activity
snapshot is always reported as **Partial history**, because an older waiting, running, queued or
finished configured run may be outside it. Branch status may read up to five 100-run pages, but
stops as soon as every configured workflow has a match. When a configured workflow has no run in
that bounded snapshot, the sidebar reports its older history as unavailable instead of claiming
that the workflow has never run. The same partial state names a configured workflow that is missing
from the repository. A repository run-query failure fails the whole refresh instead of being repeated
