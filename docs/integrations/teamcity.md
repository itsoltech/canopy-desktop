...
| `CiConfigInvalid` | A `ci` block exists but cannot be read — unparseable `.canopy/config.json` or a block shape `parseCiConfig` rejects | The CI/CD section shows the reason (front-loaded, with an "Open the configurator" button) instead of the "configure" entry, announced by the live region; `ci:config` rejects with the reason and the configurator repeats it; `ci:status` is never polled in this state                     |
