...
| `CiConfigInvalid` | A `ci` block exists but cannot be used. Two scopes, distinguished in the message: the whole `.canopy/config.json` cannot be read (bad JSON, unsupported version, a legacy tracker provider — **Save is refused**, fix the file by hand) or only the block's shape is rejected by `parseCiConfig` (re-saving replaces it) | The CI/CD section shows the reason (front-loaded, with an "Open the configurator" button) instead of the "configure" entry, announced by the live region; `ci:config` rejects with the reason and the configurator repeats it with the matching recovery; `ci:status` is never polled in this state |
