...
- **Configured**: the server row (click opens TeamCity), a highlighted **Last job** card
  ...
  that build in TeamCity). A row whose status fetch fails —
...
| `CiApiError`         | HTTP/network/API failure                                                                                                                                                                                                                                                                                                                     | When every activity query fails, **Jobs history** shows an `Error` chip (full message in its tooltip); one or two failed slices suppress the possibly-wrong sidebar summary and show their reasons in the activity window. Affected **Last job** rows show an `Unavailable` chip carrying the failure (the muted `Last job unavailable` line only appears when `ci:status` fails as a whole, which the credential banner otherwise pre-empts) |
