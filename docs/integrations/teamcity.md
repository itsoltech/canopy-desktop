...
- **Personal credentials**: access tokens are bound to their TeamCity server audience and the
  `builds.read` / `builds.trigger` capabilities in Canopy's local credential registry. A stored
  server token is used by a repository only after a native confirmation grants that canonical
  repository, exact server URL and exact set of build configuration ids access to it. Credentials
  remain managed globally in **Settings → CI connections**. See
...
- **Not configured**: a **Configure CI/CD** entry opens the per-repo configurator.
...
- **No usable token**: one banner, and nothing else. It covers a token that is missing, one the
  server rejects (a 401 on activity or status), and a stored TeamCity token that has not yet been
  ...
  **Update token**. The unapproved variant offers **Review access**, which opens the configurator
  ...
  ids. The credential verdict is read for the exact TeamCity binding together with the repository
  configuration. Saving or removing a token emits an in-process change tick, so
...
When the selected server already has a stored token but the repository scope is new or changed,
job discovery asks for explicit native approval. Saving asks again for the exact selected build
configuration ids if that scope has not already been approved. Changing the server, repository,
job ids, stored credential identity, or stored secret revision requires fresh approval. The config
grant does not silently grant the broader job-discovery scope; each consent remains independent.
Private/local-network TeamCity origins have an additional native consent. That consent is tied to
the exact normalized URL and stored only on this machine.
...
missing, the reason the primary action is disabled appears above the fields so it is encountered
before the controls it explains. That hint sits in a small reserved row so selecting a job or
branch does not shift the rest of the form.
...
- `baseUrl` - http(s) URL of the TeamCity server, optionally including its case-sensitive context
  path. Embedded credentials, query strings and fragments are rejected. All requests use the
  stored URL only; the renderer cannot redirect a repo-scoped call to another server.
...
- `buildTypes` — at most 50 entries when read; duplicate `id`s collapse to the first
  ...
  `ci:saveConfig` IPC path rejects >50 or duplicate ids outright, before consent or a config write.
...
CI/CD** entry keys on this), and `{ config: null, invalid: { scope, message } }`
...
re-initializing it. Unreadable and oversized files also propagate as file-level errors instead of
falling back to global tracker settings. Initialization is reserved for a genuinely absent file
and atomically publishes a complete temporary file through exclusive creation, so it cannot
replace a config created concurrently or expose a partial destination.
...
| `CiAuthMissing`                   | No token stored for the configured server                                                                                                                                                                                                                                                                                                    | Credential banner whose **Add credentials** (or **Update token**) action opens the machine-local credential editor without loading or changing the shared `ci` block                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `CiCredentialApprovalRequired`    | A stored TeamCity token has not been approved for this canonical repository, exact server URL and exact selected build ids                                                                                                                                                                                                                   | The sidebar replaces run/history actions with **Review access**; the native confirmation must succeed before the credential can be used                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `CiPrivateOriginApprovalRequired` | The TeamCity URL currently resolves to a private, loopback, link-local or other local-network address without prior consent for that exact URL                                                                                                                                                                                               | No token is sent and no TeamCity request is made; the next explicit connection/discovery/save action asks through a native confirmation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `CiDispatchAmbiguous`             | A TeamCity build-queue request failed after its acceptance became unknowable (transport failure, server 5xx, unreadable or malformed success response)                                                                                                                                                                                       | The run dialog stays open, tells the user TeamCity may already have accepted the build, and disables another dispatch in that dialog; the user must check TeamCity before starting again                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- `repoRoot` on every repo-scoped channel (`ci:config`, `ci:status`, `ci:trigger`,
  ...
  `ci:jobsStatus`, `ci:jobRefs`, `ci:exactJobRef`, `ci:jobParameters`, `ci:triggerJob`, `ci:runActivity`,
...
- Access tokens use a TeamCity-server audience and explicit `builds.read` /
  `builds.trigger` capabilities in the main-process-only credential registry. Repository use never
  auto-binds a compatible server credential: a native confirmation atomically creates hashed
  bindings for the canonical repository plus exact URL and, for configured use, sorted build ids.
  Canonical repository paths preserve literal backslashes on POSIX; separator normalization is
  Windows-only, so two distinct POSIX paths cannot share an approval fingerprint.
  The confirmation captures both the credential identity and an opaque secret revision; the
  transaction refuses the grant if either changes while the native dialog is open. Replacing or
  removing the server credential invalidates those grants, and an older approved token cannot be
  auto-selected as the server default. Revoked repository bindings are never recreated by a late
  result from an in-flight request made with the old token. Secrets are
  ...
  [Integration credentials](credentials.md).
- For repo-scoped calls (status, trigger, parameters, branches, activity) the request target
  always comes from the repo config. The `ci:build` observer also carries its expected server
  identity, but the main process requires an exact match and still uses the stored URL, so a
  ...
  every request times out after 15 s. Response bodies are streamed with a 2 MiB cap;
  oversized bodies are cancelled before they can grow without bound. Stored tokens are looked up by the exact
  `baseUrl` - no stored credential for an unknown URL means no token is sent.
- The build-completion observer sends the server identity it started with. The main process
  requires it to still equal the repository's TeamCity URL and verifies the returned build type
  remains in the configured allowlist before reporting status.
...
- Every candidate-token connection test also requires a main-process-owned native confirmation
...
- Before any TeamCity request, the main process resolves the server hostname. An invalid or
  ...
  the actual Undici socket lookup, using the selected address for that connection, so a DNS change
  between preflight and connect cannot bypass consent. Redirects remain disabled.
- TeamCity queue dispatch is attempted exactly once. Once the request could have reached TeamCity,
  transport failures, 5xx responses and unreadable/malformed success bodies become
  `CiDispatchAmbiguous`; Canopy does not automatically retry and disables a second submission in
  the same dialog. That lock is terminal for the dialog instance and survives back navigation,
  job changes and ref changes; the user must verify TeamCity before opening a fresh run flow.
- `password` prompt parameters never reach the renderer: their server-side value is
  ...
  secret. Mutation failures use a fixed status-based message rather than reflecting a provider
  response body that could contain submitted parameter values.
...
| Sidebar               | `src/renderer/src/components/sidebar/CiSection.svelte`, `src/renderer/src/components/sidebar/ciSectionState.svelte.ts`, `src/renderer/src/components/sidebar/_partials/CiConfiguredContent.svelte`, `src/renderer/src/components/sidebar/_partials/CiCredentialBanner.svelte`, `src/renderer/src/components/sidebar/_partials/CiActivityEntry.svelte`, `src/renderer/src/components/ci/CiLastJobCard.svelte`, `src/renderer/src/components/ci/CiLastRunCard.svelte`, `src/renderer/src/components/ci/CiLastStatusCard.svelte`, `src/renderer/src/components/sidebar/ProjectTreeSection.svelte` |
| Dialogs               | `src/renderer/src/components/ci/CiRunDialogRouter.svelte`, `src/renderer/src/components/ci/CiRunDialog.svelte` (shared TeamCity/GitHub Actions run flow), `src/renderer/src/components/ci/CiRunConfirmation.svelte`, `src/renderer/src/components/ci/CiRunParameterFields.svelte`, `src/renderer/src/components/ci/CiActivityModal.svelte`, `src/renderer/src/components/ci/TeamCityActivityRow.svelte` (activity rendered from `MainLayout`)                                                                                                                                                  |
| Per-repo configurator | `src/renderer/src/components/preferences/ProjectCiModal.svelte`, `src/renderer/src/components/preferences/projectCiModalState.svelte.ts`, `src/renderer/src/components/preferences/_partials/ProjectCiServerSection.svelte`, `src/renderer/src/components/preferences/_partials/ProjectCiCredentialSection.svelte`, `src/renderer/src/components/preferences/_partials/ProjectCiJobsSection.svelte`, `src/renderer/src/components/preferences/_partials/ProjectCiFooter.svelte`, `src/renderer/src/components/ci/CiJobPicker.svelte`                                                           |
| Settings              | `src/renderer/src/components/preferences/CiConnectionsPrefs.svelte` (CI connections), `src/renderer/src/components/preferences/_partials/CiConnectionRow.svelte` (saved connection), `src/renderer/src/components/preferences/_partials/CiServerForm.svelte` (add/edit server form)                                                                                                                                                                                                                                                                                                            |
